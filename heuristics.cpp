...
void maximal_spantrusses(string dataset_path) {
    ...
    vector<int> K(tgraph.timestamps + 1, -1);
    vector<int> TM;
            ...
            for(auto e : temporal_edges) {
				...
				for(auto j = graph[u].begin(); j != graph[u].end(); j++) {
					...
					if(graph[v].find(w) != graph[v].end()) {
						...
						if(graph[u][v] > k_ii || graph[w][v] > k_ii || graph[w][u] > k_ii) do_decomposition = true;
					}
				}
            }

			if(!do_decomposition) continue;
